...
```bash
npx @fictjs/shadcn@latest add @v0/dashboard
```

or multiple resources at once:

```bash
npx @fictjs/shadcn@latest add @acme/header @lib/auth-utils @ai/chatbot-rules
```
...
```bash
npx @fictjs/shadcn@latest add @acme/button
```
...
> **Note:** The `{name}` placeholder in the URL is automatically parsed and replaced with the resource name when you run `npx @fictjs/shadcn@latest add @namespace/resource-name`. For example, `@acme/button` becomes `https://registry.acme.com/resources/button.json`. See [URL Pattern System](#url-pattern-system) for more details.
...
```bash
npx @fictjs/shadcn@latest view @acme/button
```
...
When you run `npx @fictjs/shadcn@latest add @namespace/resource`, the CLI does the following:
...
```bash
npx @fictjs/shadcn@latest add @acme/auth @custom/login-form
```
...
```bash
npx @fictjs/shadcn@latest add @my-company/custom-button
```
...
npx @fictjs/shadcn@latest add @acme/button @lib/utils @ai/prompt

# Install from URL directly
npx @fictjs/shadcn@latest add https://registry.example.com/button.json

# Install from local file
npx @fictjs/shadcn@latest add ./local-registry/button.json
```
...
npx @fictjs/shadcn@latest view @v0/dashboard @shadcn/card

# View from URL
npx @fictjs/shadcn@latest view https://registry.example.com/button.json
```
...
npx @fictjs/shadcn@latest search @v0

# Search with query
npx @fictjs/shadcn@latest search @acme --query "auth"

# Search multiple registries
npx @fictjs/shadcn@latest search @v0 @acme @lib

# Limit results
npx @fictjs/shadcn@latest search @v0 --limit 10 --offset 20

# List all items (alias for search)
npx @fictjs/shadcn@latest list @acme
```
...
```bash
npx @fictjs/shadcn@latest add @non-existent/component
```
