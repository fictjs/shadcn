...
```bash
npx @fictjs/shadcn@latest add [component]
```
...
```bash
npx @fictjs/shadcn@latest view [item]
```

You can view multiple items at once:

```bash
npx @fictjs/shadcn@latest view button card dialog
```

Or view items from namespaced registries:

```bash
npx @fictjs/shadcn@latest view @acme/auth @v0/dashboard
```
...
```bash
npx @fictjs/shadcn@latest search [registry]
```

You can search with a query:

```bash
npx @fictjs/shadcn@latest search @shadcn -q "button"
```

Or search multiple registries at once:

```bash
npx @fictjs/shadcn@latest search @shadcn @v0 @acme
```
...
```bash
npx @fictjs/shadcn@latest build
```
...
```bash
npx @fictjs/shadcn@latest build --output ./public/registry
```
...
```bash
npx @fictjs/shadcn@latest migrate [migration]
```
...
```bash
npx @fictjs/shadcn@latest migrate rtl
```
...
npx @fictjs/shadcn@latest migrate rtl src/components/ui/button.tsx

# Migrate files matching a glob pattern
npx @fictjs/shadcn@latest migrate rtl "src/components/ui/**"
```
...
```bash
npx @fictjs/shadcn@latest migrate radix
```
...
```bash
# Migrate a specific file.
npx @fictjs/shadcn@latest migrate radix src/components/ui/dialog.tsx

# Migrate files matching a glob pattern.
npx @fictjs/shadcn@latest migrate radix "src/components/ui/**"
```
