---
title: August 2024 - npx @fictjs/shadcn init
description: Complete CLI rewrite with support for all major React frameworks.
date: 2024-08-31
---

The new CLI is now available. It's a complete rewrite with a lot of new features and improvements. You can now install components, themes, hooks, utils and more using `npx @fictjs/shadcn add`.
...
3. You can also install remote components using url. `npx @fictjs/shadcn add https://acme.com/registry/navbar.json`.
4. We have also improve the init command. It does framework detection and can even init a brand new Next.js app in one command. `npx @fictjs/shadcn init`.
...
```bash
npx @fictjs/shadcn init sidebar-01 login-01
```
