...
- **Same CLI workflow** - Run `npx @fictjs/shadcn add` and the CLI will pull the correct block variant based on your project configuration.

If you've already set up your project with `npx @fictjs/shadcn create`, blocks will automatically use your chosen library. No additional configuration needed.

```bash
npx @fictjs/shadcn@latest add login-01
```
