...
```bash
npx @fictjs/shadcn add @acme/button @internal/auth-system
```
...
```bash
npx @fictjs/shadcn view @acme/auth-system
```

2. Search items from registries

```bash
npx @fictjs/shadcn search @tweakcn -q "dark"
```

3. List all items from a registry

```bash
npx @fictjs/shadcn list @acme
```
...
```bash
npx @fictjs/shadcn@latest mcp init
```
